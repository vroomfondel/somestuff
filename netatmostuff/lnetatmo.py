...
TYPES = {
    ...
    "NAPlug": [
        ...
        "Energy",
    ],  # A smart thermostat exist of a thermostat module and a Relay device
...
UNITS = {
    ...
    "Health index": {0: "Healthy", 1: "Fine", 2: "Fair", 3: "Poor", 4: "Unhealthy"},  # Homecoach
    "Wifi status": {86: "Bad", 71: "Average", 56: "Good"},  # Wifi Signal quality
}
...
class ClientAuth:
    ...
    def accessToken(self):
        print(f"accessToken::refreshToken expires: {datetime.fromtimestamp(self.expiration)}")
    ...
    def renew_token(self):
        ...
        self.expiration = int(resp["expire_in"] + time.time() - 60 * 10)  # -10 min.

        print(f"renewToken::refreshToken expires: {datetime.fromtimestamp(self.expiration)}")
...
class WeatherStationData:
    ...
    def __init__(self, authData, home=None, station=None):
        ...
        if station:
            ...
            self.default_station = [
                v["station_name"] for k, v in self.stations.items() if v["home_name"] == self.default_home
            ][0]
        self.modules = {}
    ...
    def modulesNamesList(self, station=None):
        ...
        if not s:
            raise NoDevice("No station with name or id %s" % station)
        self.default_station = s["station_name"]
    ...
    def lastData(self, station=None, exclude=0):
        ...
        if ds.get("time_utc", limit + 10) > limit:
            ...
            lastD[s["module_name"]]["When"] = (
                lastD[s["module_name"]].pop("time_utc") if "time_utc" in lastD[s["module_name"]] else time.time()
            )
    ...
    def MinMaxTH(self, module=None, frame="last24"):
        ...
        if module and module != s["module_name"]:
            ...
            resp = self.getMeasure(
                device_id=s["_id"], scale="max", mtype="Temperature,Humidity", date_begin=start, date_end=end
            )
...
class HomeData:
    ...
    def getCameraPicture(self, image_id, key):
        ...
        """
        postParams = {"access_token": self.getAuthToken, "image_id": image_id, "key": key}
    ...
    def updateEvent(self, event=None, home=None):
        ...
        home_data = self.homeByName(home)
        postParams = {"access_token": self.getAuthToken, "home_id": home_data["id"], "event_id": event["id"]}
    ...
    def presenceSetAction(self, camera=None, home=None, cid=None, eventType=_PRES_DETECTION_KIND[0], action=2):
...
class WelcomeData(HomeData):
    ...
    warnings.warn(
        "The 'WelcomeData' class was renamed 'HomeData' to handle new Netatmo Home capabilities", DeprecationWarning
    )
...
class HomeCoach:
    ...
    def checkNotUpdated(self, res, hid, delay=3600):
        ...
        if time.time() - res["When"] > delay:
            ret.append({hid: "Device Not Updated"})
        return ret if ret else None

    def checkUpdated(self, res, hid, delay=3600):
        ret = []
        if time.time() - res["When"] < delay:
            ret.append({hid: "Device up-to-date"})
        return ret if ret else None
...
def postRequest(topic, url, params=None, timeout=10):
    ...
    ret = json.loads(data.decode("utf-8")) if "application/json" in returnedContentType else data

    # print(f"lnetatmo::postRequest::{type(ret)=}")
