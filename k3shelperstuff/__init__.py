"""k3shelperstuff — helpers around a K3s/Kubernetes cluster.

Bundles several standalone tools:

* :mod:`k3shelperstuff.update_local_k3s_keys` — sync local ``~/.kube/config``
...
``requests``/``urllib3``) log via the stdlib ``logging`` module, so
...
def print_banner(title: str) -> None:
    ...
    Args:
        title: Tool-specific banner title (this package bundles several CLIs, so
            each one passes its own, e.g. ``"keel_drift"``).
    """
    startup_rows = [
        ["version", __version__],
        ["buildtime", os.getenv("BUILDTIME", "unknown")],
        ["github", "https://github.com/vroomfondel/somestuff/tree/main/k3shelperstuff"],
