"""broadlinkstuff — Broadlink RM4 IR/RF blaster tooling.

Talk to Broadlink RM4 devices: address them without a broadcast, learn IR/RF
codes, send stored or raw codes, and — for Midea/Coolix air conditioners —
compute the complete mode × fan × temperature matrix instead of learning all
280 combinations one by one in front of the device.

* :mod:`broadlinkstuff.broadlinkhelper` — device fleet, Coolix codec, Typer CLI

Central logging setup lives here (loguru), mirroring ucmstuff's setup. This
package (and third-party libraries such as ``broadlink``) logs via the stdlib
...
class _InterceptHandler(logging.Handler):
    ...
    The modules in this package use ``logging.getLogger(...)``; installing this
...
def configure_logging(
    ...
    The sink is stderr, deliberately: the CLI's payload (learned hex codes,
    config blocks, code listings) goes to stdout, so it survives a pipe without
    log lines mixed in.
...
def print_banner(title: str) -> None:
    ...
    Args:
        title: Tool-specific banner title, e.g. ``"broadlinkhelper"``.
    """
    startup_rows = [
        ["version", __version__],
        ["buildtime", os.getenv("BUILDTIME", "unknown")],
        ["github", "https://github.com/vroomfondel/somestuff/tree/main/broadlinkstuff"],
