"""oepnvstuff — GTFS / GTFS-Realtime public-transport (ÖPNV) tooling.

Validates whether an open GTFS-Realtime feed actually carries real-time data
(delays / actual times) for configurable transit lines at a configurable
station, either as a one-shot check or as a polling watch loop with pluggable
``on_*`` handlers (console reporting, MQTT publishing, …).

Central logging setup lives here (loguru), mirroring ucmstuff's setup. The
modules in this package (and third-party libraries such as ``requests``/
``urllib3`` and ``paho-mqtt``) log via the stdlib ``logging`` module, so
...
def print_banner() -> None:
    ...
    startup_rows = [
        ...
        ["github", "https://github.com/vroomfondel/somestuff/tree/main/oepnvstuff"],
    ...
    title = "oepnv GTFS-realtime checker starting up"
